...
fn bytes_per_pixel(format: ColorFormat) -> u32 {
  match format {
...
impl Frame {
  ...
  pub fn crop(&self, start_x: u32, start_y: u32, end_x: u32, end_y: u32) -> Result<Self> {
    if start_x >= end_x || start_y >= end_y || end_x > self.width || end_y > self.height {
      return Err(error("Invalid crop rectangle"));
    }

    let bpp = bytes_per_pixel(self.color_format.clone());
    let width = end_x - start_x;
    let height = end_y - start_y;
    let row_pitch = width * bpp;
    let mut buffer = vec![0; (row_pitch * height) as usize];

    for row in 0..height {
      let source_start = ((start_y + row) * self.row_pitch + start_x * bpp) as usize;
      let source_end = source_start + row_pitch as usize;
      let destination_start = (row * row_pitch) as usize;
      buffer[destination_start..destination_start + row_pitch as usize]
        .copy_from_slice(&self.buffer[source_start..source_end]);
    }
  ...
  #[napi]
  pub fn save_as_image(&self, path: String, format: ImageFormat) -> Result<()> {
    std::fs::write(path, self.encode(format)?.as_ref()).map_err(error)
  }
}
...
impl GraphicsCaptureApiHandler for CaptureHandler {
  ...
  fn on_frame_arrived(
    ...
    let row_pitch = width * bytes_per_pixel(color_format.clone());
...
impl WindowsCapture {
  ...
  #[napi]
  pub fn start(&self) -> Result<CaptureControl> {
    let flags = (self.on_frame_arrived.clone(), self.on_closed.clone());
